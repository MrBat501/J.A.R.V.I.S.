...
// ===== PERSISTENCIA LOCAL (guardado en el navegador) =====
...
// ===== DATOS: se cargan de localStorage, o de estos ejemplos la primera vez =====
const seedTasks = [
...
function loadFromStorage() {
  try {
    ...
    tasks = rawTasks ? JSON.parse(rawTasks) : seedTasks;
    contacts = rawContacts ? JSON.parse(rawContacts) : seedContacts;

    // Si es la primera vez (no había nada guardado), sembramos los ejemplos.
    if (!rawTasks) saveTasks();
    if (!rawContacts) saveContacts();
  } catch (err) {
    storageAvailable = false;
    tasks = seedTasks;
    contacts = seedContacts;
    showConnBanner(
      "⚠ Este navegador no permite guardar datos localmente (por ejemplo, en modo incógnito). Verás datos de ejemplo que no se guardarán."
    );
  }
}
